...
$objPHPExcel->getProperties()->setCreator("aizensouxx")
        ...
        ->setTitle("Reporte De Acuerdos Administrativos")
        ->setSubject("Reporte")
        ->setDescription("")
        ->setKeywords("")
        ->setCategory("");
...
$y = 5;
$columna = "B";
$objPHPExcel->setActiveSheetIndex(0)->setCellValue("A" . $y, "Numero Acuerdo");
...
if (sizeof($registros_filtrados) == 1) {
    $sql = "SELECT * FROM acuerdoAdministrativo WHERE numContrato = '$registros_filtrados[0]'";
} else {
    $reg_count=1;
    $sql = "SELECT * FROM acuerdoAdministrativo WHERE ";
